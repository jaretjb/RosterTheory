...
| TA-1313 | P0 | REVIEW | Repair Trade Finder's weekly projection scoring-scope mismatch without relabeling provider data or weakening ranking scope: use raw-stat projections only when they can be scored under the league's actual Sleeper rules, preserve the provider-declared format in provenance, and keep missing or ambiguous data unavailable. Update the affected league's ignored provisional target policy to the supported schema without promoting calibration or transferring policy to another league. | TA-1308, TA-1312 | Current provider and Trade scoring contracts only | Synthetic mismatch fixtures pass; exact ranking scope is unchanged. League Beta's ignored provisional policy loads after obsolete fields are removed. The read-only rerun passes the projection-scope gate and evaluates 52 packages, but remains INCOMPLETE: two rosters each have one missing Week 3 player projection, so there are no offer recommendations. All 799 tests and Ruff pass; no Sleeper write. Awaiting PR review. |
