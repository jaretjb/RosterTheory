...
Status: TA-1312 complete; TA-1310 planned
...
| TA-1312 | P0 | COMPLETE | Make the Trade ROS expert panel resilient to normal in-season contributor scarcity. Prefer the three best historically accurate, fresh experts who actually contribute to every current ROS position feed, accept two as an explicitly degraded panel, renormalize only the available selected weights, and continue to fail closed below two. Preserve the independent Latest ECR market board and do not import Waiver acquisition scoring, weekly-rank weighting, or Waiver Wire policy into Trade. | TA-1309, WA-024 | R§3.3-3.4, §6.2, §8.1-8.3; D§5.3-5.4, §8.4, §13-15 | Synthetic refresh/import/validation fixtures prove 3=READY, 2=DEGRADED, 1=blocked, actual-contributor filtering, normalized weights, visible provenance, and a separate market ECR board. League Beta's cached read-only refresh completed with three actual contributors and zero provider calls; League Alpha passed expert selection before an unrelated inactive-player coverage gate. All 666 tests and Ruff pass; details are in `docs/COMPLETED_TRADE_ASSISTANT_TA_1312.md`. |
| TA-1313 | P0 | COMPLETE | Repair Trade Finder's weekly projection scoring-scope mismatch without relabeling provider data or weakening ranking scope: use raw-stat projections only when they can be scored under the league's actual Sleeper rules, preserve the provider-declared format in provenance, and keep missing or ambiguous data unavailable. Update the affected league's ignored provisional target policy to the supported schema without promoting calibration or transferring policy to another league. | TA-1308, TA-1312 | Current provider and Trade scoring contracts only | Implementation complete; delivery through PR #25. Synthetic mismatch fixtures pass; exact ranking scope is unchanged. League Beta's ignored provisional policy loads after obsolete fields are removed. The prior read-only rerun passes the projection-scope gate and evaluates 52 packages, but remains INCOMPLETE: two rosters each have one missing Week 3 player projection, so there are no offer recommendations. Revalidated against PR #26: all 802 tests, Ruff, and tracked-tree/index privacy gates pass; no additional provider refresh or Sleeper write. |
