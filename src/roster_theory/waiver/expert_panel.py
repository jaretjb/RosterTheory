...
MAXIMUM_UPDATE_AGE = timedelta(days=14)
...
def _accuracy_score(
    ...
    return (
        round(
            LATEST_ACCURACY_WEIGHT * latest + PRIOR_ACCURACY_WEIGHT * prior,
            6,
        ),
        latest,
        prior,
    ...
    *,
...
) -> ExpertPoolResolution:
    """Select the best accurate, current contributors in the usable ROS feed.

    This is Waiver-owned evidence selection. Trade continues to use its saved
    five-expert pool. Ranking availability comes from the actual ROS payload,
    while accuracy and update timestamps come from the current expert directory.
    """

    if now.tzinfo is None:
        raise ValueError("Waiver ROS panel selection time must be timezone-aware")
    if preferred_size < minimum_size or minimum_size < 2:
        raise ValueError("Waiver ROS panel requires at least two preferred experts")
    ...
    for expert_id in sorted(
        ...
        row: dict[str, Any] = {
            ...
            "position_updates": dict(expert.position_updates) if expert else {},
    ...
    if len(selected) < minimum_size:
        ...
        raise CoverageIncomplete(
            f"Waiver ROS panel found {len(selected)} eligible contributor(s); "
    ...
    evidence: Mapping[str, Any] = {
        "schema_version": "roster-theory.waiver-ros-panel/v1",
        ...
        "candidates": audit,
        "disagreement_evidence": "preserved_per_player_in_selected_rank_contributions",
        "sleeper_write_performed": False,
    }
    return ExpertPoolResolution(members=members, evidence=evidence)


class WaiverRosPanelSelector:
    def __init__(self, league_key: str) -> None:
        self.league_key = league_key

    def __call__(
        self, inputs: ValueInputsLike, now: datetime
    ) -> ExpertPoolResolution:
        return select_waiver_ros_panel(inputs, now, league_key=self.league_key)
